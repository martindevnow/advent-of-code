...
const isBefore = (date1, date2): boolean => {
  return !![...date1].reduce(
    (acc, num, index) => {
    ...
    { equal: true, smaller: null, larger: null }
  ).smaller;
};
...
const calculatedData = sorted.reduce(
  (acc: Acc, entry) => {
    // Setup ACC
    ...
    if (isChangeGuard(entry.action)) {
      ...
      if (
        !acc.guards[acc.curr.guard] ||
        acc.guards[acc.curr.guard].length === 0
      ) {
    ...
    if (isWakeUp(entry.action)) {
      if (acc.curr.status === "asleep") {
        ...
        acc.curr.time = getMinutes(entry.theDate);
        acc.curr.status = "awake";
        acc.guards[acc.curr.guard] = acc.guards[acc.curr.guard].map(
          (times, minute) => {
...
const sleepyGuard = Object.keys(calculatedData.guards).reduce(
  (acc: any, currGuardKey) => {
    if (acc === null) {
      return currGuardKey;
    }

    if (
      calculatedData.guards[currGuardKey].reduce(sum, 0) >
      calculatedData.guards[acc].reduce(sum, 0)
    ) {
      return currGuardKey;
    }
    return acc;
...
const sleepiestMinute = calculatedData.guards[sleepyGuard].reduce(
  (acc, curr, index) => {
    if (acc === null) {
      return index;
    }
...
console.log(sleepiestMinute, sleepyGuard, getJustNumber(sleepyGuard));
console.log(sleepiestMinute * getJustNumber(sleepyGuard));

// Part 2

const guards: any = Object.keys(calculatedData.guards).reduce(
  (acc, guardKey) => {
    return {
      ...acc,
      [guardKey]: calculatedData.guards[guardKey].reduce(
        (acc, timesAsleep, minute) => {
...
const guard = Object.keys(guards).reduce((acc: any, guardKey, index) => {
  if (acc === null) {
    return guardKey;
  }
