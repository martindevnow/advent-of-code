...
const getPair = (num, invert = false) => {
  const pair = String.fromCharCode(num + 64) + String.fromCharCode(num + 64 + 32);
  return !invert ? pair : pair.split('').reverse().join('');
}

// console.log(getPair(1), getPair(26))

const pairList = new Array(26 * 2).fill().map((_, index) => {
...
const charList = new Array(26).fill().map((_, index) => String.fromCharCode(index + 1 + 64));
