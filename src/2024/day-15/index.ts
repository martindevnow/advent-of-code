...
const input: string = utils.readFileAbsolute(__dirname + "/data.txt");
...
function walkVertical(deltaRow) {
  ...
  if (!mayMoveBoxVertical(futureRow, botCol, deltaRow)) {
    return;
  }
...
function walkHorizontal(deltaCol) {
  ...
  while (true) {
    // searchs for the first free space ahead
  ...
  while (true) {
    // moves everything in the segment from the bot to the first free space ahead
...
function mayMoveBoxVertical(boxRow, boxCol, deltaRow) {
  // recursive function
  ...
  if (futureSpotLeft == "[") {
    if (!mayMoveBoxVertical(futureRow, boxCol, deltaRow)) {
      return false;
    }
  }
  ...
  if (futureSpotRight == "[") {
    if (!mayMoveBoxVertical(futureRow, boxCol + 1, deltaRow)) {
      return false;
    }
  }
...
function moveBoxVertical(boxRow, boxCol, deltaRow) {
  // movement already checked // recursive function
